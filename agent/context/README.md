...
| Type | File | Purpose |
|------|------|---------|
| `ContextManager` | [manager.py](agent/context/manager.py) | Entry point — add turns, assemble context, trigger compression |
| `ConversationRecord` | [schema.py](agent/context/schema.py) | Stored turn — full Message fidelity including tool_calls |
| `SummaryRecord` | [schema.py](agent/context/schema.py) | Compressed summary with `end_turn` anchor |
| `WorkingContext` | [schema.py](agent/context/schema.py) | Assembled LLM-visible context with `to_messages()` |
| `ContextSnapshot` | [snapshot.py](agent/context/snapshot.py) | Token counting, segment splitting, compression triggers |
| `RoastState` | [roast.py](agent/context/roast.py) | Pure functions for roast lifecycle (assignment, staleness, queries) |
| `ContextCompressor` | [compression/compressor.py](agent/context/compression/compressor.py) | Unified compression pipeline (L2+L3+L4) |
...
Prompt: [l2_extract_facts.j2](agent/context/prompts/l2_extract_facts.j2)
...
Prompts: [l3_summary_initial.j2](agent/context/prompts/l3_summary_initial.j2), [l3_summary_merge.j2](agent/context/prompts/l3_summary_merge.j2)
