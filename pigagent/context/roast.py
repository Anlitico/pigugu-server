...
class RoastState:
    ...
    def assign_roast_instance_id(history: list[ConversationRecord], current: ConversationRecord) -> str | None:
        ...
        prev = RoastState.current_roast_instance_id(history)

        if prev and not RoastState._is_stale(history, current):
            current.roast_instance_id = prev
            return prev

        current.roast_instance_id = None
        return None

    # ── State queries ────────────────────────────────────────────────

    @staticmethod
    def current_roast_instance_id(records: list[ConversationRecord]) -> str | None:
        if records:
            return records[-1].roast_instance_id
        return None

    @staticmethod
    def is_active(records: list[ConversationRecord]) -> bool:
        return bool(records) and records[-1].roast_instance_id is not None
    ...
    def _is_stale(history: list[ConversationRecord], current: ConversationRecord | None) -> bool:
        ...
        rid = RoastState.current_roast_instance_id(history)
        if not rid:
            return True
        for r in history:
            if r.roast_instance_id == rid:
