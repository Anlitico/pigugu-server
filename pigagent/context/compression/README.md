...
Supporting modules:
- `pigagent/context/snapshot.py` — `ContextSnapshot` wraps record list with token counting and segment analysis
- `pigagent/context/roast.py` — `RoastState` for roast lifecycle (assignment, staleness, queries)
- `pigagent/context/storage/redis.py` — Redis I/O (turns, summaries, roast data, compression lock)
- `pigagent/context/storage/pg.py` — PostgreSQL I/O (facts, profile persistence)
